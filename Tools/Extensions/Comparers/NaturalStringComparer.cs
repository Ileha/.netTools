namespace Tools.Extensions.Comparers
{
    ...
    {
        private readonly Func<T, string> _stringExtractor;
        private readonly NaturalStringComparer _stringComparer = new NaturalStringComparer();

        public NaturalStringComparer(Func<T, string> stringExtractor)
        {
            _stringExtractor = stringExtractor ?? throw new ArgumentNullException(nameof(stringExtractor));
        }

        public int Compare(T x, T y)
        {
            var stringX = _stringExtractor(x);
            var stringY = _stringExtractor(y);

            return _stringComparer.Compare(stringX, stringY);
        }
    }
